...
type BlameOptions struct {
	...
	C bool
	// --score-debug
	// Include debugging information related to the movement of lines between files (see -C) and lines moved within a file (see -M).
	ScoreDebug bool
	// -f
	// --show-name
	// Show the filename in the original commit when different due to rename detection.
	ShowName bool
	// -n
	// --show-number
	...
	// -e
	// --show-email
	// Show the author email instead of the author name. Can also be controlled via blame.showEmail config option.
	ShowEmail bool
	// -w
	// Ignore whitespace when comparing the parent’s version and the child’s to find where the lines came from.
	W bool
	// --abbrev=<n>
	// Instead of using the default 7+1 hexadecimal digits as the abbreviated object name, use <m>+1 digits, where <m> is at least <n> but ensures the commit object names are unique.
	Abbrev uint64
	// --
...
func BlameCmd(opts *BlameOptions) *exec.Cmd {
	args := []string{"blame"}
	...
	if opts.ScoreDebug {
		args = append(args, "--score-debug")
	}
	if opts.ShowName {
		args = append(args, "--show-name")
	}
	if opts.ShowNumber {
		args = append(args, "--show-number")
	}
	if opts.S {
		args = append(args, "-s")
	}
	if opts.ShowEmail {
		args = append(args, "--show-email")
	}
	if opts.W {
		args = append(args, "-w")
	}
